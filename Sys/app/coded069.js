...
le._apps['code'] = {
    ...
    exec: function(url, opt) {
      ...
      $loader([
        ...
        var pos_line = 0;
        var pos_char = 0;
        var filePath = url ? url.replace(/:(\d+)(?::(\d+))?$/, function(_, l, c) {
          pos_line = l || 0;
          pos_char = c || 0;
          return '';
        }) : '';
        ...
        $io.obj.all(CodeMirror.keyMap.sublime, function(val, key) {
          ...
          instructions += '<tr><td>' + text + '</td><td> : ' + key + '</td></tr>';
        });
        instructions += '</table>';
        ...
        $editor.call(self, {
          ...
          create: function(el) {
            ...
            function fixCustomFont() {
              ...
              // so... lots of refresh :(
              /////////////////////////////////////////////////////////////////////////////
              setTimeout(function() { codMi.refresh() }, 1);
              setTimeout(function() { codMi.refresh() }, 10);
              setTimeout(function() { codMi.refresh() }, 100);
              setTimeout(function() { codMi.refresh() }, 500);
              setTimeout(function() { codMi.refresh() }, 1000);
              setTimeout(function() { codMi.refresh() }, 1500);
            }
            ...
            return {
              readFile: function(val) {
                ...
                codMi.setValue(typeof val == 'string' ? val : JSON.stringify(val));
                ...
                var coords = codMi.charCoords({line: pos_line-1, ch: pos_char-1}, "local");
                codMi.scrollTo(null, (coords.top + coords.bottom - myHeight) / 2);
                setTimeout(function() { codMi.focus(); }, 100);
              },
