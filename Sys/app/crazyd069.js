system42.on('apps:ready', function(le) { 'use strict'
  ...
  le._apps['crazy'] = {
    ...
    exec: function(url, opt) {
      ...
      $loader([
        '/c/libs/MIDIFile/dist/MIDIFile.js'
      ], function() {
        ...
        var bsod = {
          show: function() {bsodEl.classList.remove('hide')},
          hide: function() {bsodEl.classList.add('hide')}
        }
        var login = {
          show: function() {loginEl.classList.remove('hide')},
          hide: function() {loginEl.classList.add('hide')}
        }
        ...
        var trigo = {
          cnt: 0,
          exec: function (total) {
            var xy = pointFromPointAngleRadius([w2, h2], (this.cnt * (360 / total)) - 90, cont.offsetWidth / 5)
            this.cnt++
            var cnt = this.cnt
            return {
              left: xy[0] - (380/2),
        ...
        var OK = {
          cnt: 0,
          exec: function () {
            var xy = pointFromPointAngleRadius([w2, h2], (this.cnt * (360 / 10)) - 90, cont.offsetWidth / 5)
            this.cnt++
            var cnt = this.cnt
            return {
              left: xy[0] - (380/2),
        ...
        .then(function (assets) {
          ...
          midi.arrayBuffer().then(function(buffer) {
            ...
            midiFile.header.setTicksPerBeat((midiFile.header.getTicksPerBeat() / 120) * 170)
            ...
            for(var i = 0, j = events.length; i < j; i++) {
              ...
              //////////////////////
              !(function (ev, i) {
                if (i === 0) setTimeout(function () { track.play() }, curTime + ev.playTime)
                setTimeout(function () {
