      ...
      function load() {
        ...
        function setFolder(url) {
          ...
          $io.obj.each(folderObj, function(val, key) {
            if (/\.(bmp|gif|png|jpe?g|svg|ico|cur|tiff?)/.test(key)) folderKeys.push(key);
          });
          folderKeys.sort();
        ...
        //console.log(url)
        var winInst = $window.call(that, $extend({
          title: url,
          ...
          contextmenu: {'after:FX': [
             {name: '---'}
            ,{name: 'View', items: [
               {name: 'Pixelated', key: 'p', checkbox:true, selected:false, action: function() {
                console.log('pixelated')
                html.classList.toggle('pixelated');
              }}
              ,{name: '---'}
              ,{name: 'Cover', radio:'View', selected:true, action: function() {
                backgroundSize = html.style.backgroundSize = 'cover';
              }}
              ,{name: 'Contain', radio:'View', action: function() {
                backgroundSize = html.style.backgroundSize = 'contain';
              }}
              ,{name: 'Fit', radio:'View', action: function() {
                backgroundSize = html.style.backgroundSize = '100% 100%';
              }}
              ,{name: 'Scale', radio:'View', action: function() {
                backgroundSize = html.style.backgroundSize = 'auto auto';
              }}
            ]}
            ,{name: 'Layer', items: [
               {name: 'Previous', key: 'left', action: function() {
                currentIndex--; changeImage();
              }}
              ,{name: 'Next', key: 'right', action: function() {
                currentIndex++; changeImage();
              }}
              ,{name: 'Random', key: 'up', action: function() {
                var random = ~~(Math.random() * folderKeys.length);
                currentIndex = random !== currentIndex ? random : random + 1;
                changeImage();
              }}
              ,{name: 'Random Folder', key: 'down', action: function() {
                var rDir = pickRandomFolder(le._files.c.files.images.gif.vj);
                var rFile = pickRandomFile(le._files.c.files.images.gif.vj[rDir]);
                setFolder('c/files/images/vj/'+rDir+'/'+rFile);
                var random = ~~(Math.random() * folderKeys.length);
              ...
              ,{name: '---'}
              ,{name: 'Duplicate', key: 'ctrl+alt+space', action: function() {
                //$exe('layer ' + url)
          ...
          bodyClass: isLayer ? '' : 'skin_inset_deep ui_layout app_imageviewer',
          borderTopWidth: 0,
          borderBottomWidth: 0,
          borderLeftWidth: 0,
          borderRightWidth: 0,
